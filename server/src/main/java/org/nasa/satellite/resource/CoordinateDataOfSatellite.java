...
import org.nasa.ws.proxy.CoordinateComponent;
import org.nasa.ws.proxy.CoordinateData;
import org.nasa.ws.proxy.CoordinateOptions;
import org.nasa.ws.proxy.CoordinateSystem;
import org.nasa.ws.proxy.DataFileRequest;
import org.nasa.ws.proxy.DataResult;
import org.nasa.ws.proxy.DateFormat;
import org.nasa.ws.proxy.DegreeFormat;
...
import org.nasa.ws.proxy.LatLonFormat;
import org.nasa.ws.proxy.LocationFilter;
import org.nasa.ws.proxy.LocationFilterOptions;
import org.nasa.ws.proxy.MapProjection;
import org.nasa.ws.proxy.MapProjectionGraphOptions;
import org.nasa.ws.proxy.MappedRegionFilterOptions;
import org.nasa.ws.proxy.OrbitGraphOptions;
import org.nasa.ws.proxy.OutputOptions;
import org.nasa.ws.proxy.PolarMapOrientation;
...
import org.nasa.ws.proxy.SatelliteSituationCenterInterface;
import org.nasa.ws.proxy.SatelliteSituationCenterService;
import org.nasa.ws.proxy.SatelliteSpecification;
import org.nasa.ws.proxy.SpaceRegion;
import org.nasa.ws.proxy.SpaceRegionsFilterOptions;
import org.nasa.ws.proxy.TimeFormat;
import org.nasa.ws.proxy.TimeSeriesGraphOptions;
...
//					+ sat.getEndTime() + "  " + "  " + sat.getGeometry() + "  "
...
//		};
	...
	private static void print(SatelliteData data) {
		...
		List<FootpointRegion> rtr = data.getRadialTraceFootpointRegion();
		...
		printHeading(coords, radialLength, magneticStrength, ns, bs, mp, lv,
				il, sr, rtr, nbtr, sbtr, bGseX, bGseY, bGseZ);

		print(time, coords, radialLength, magneticStrength, ns, bs, mp, lv, il,
				sr, rtr, nbtr, sbtr, bGseX, bGseY, bGseZ, 5);

	}

	private static void printHeading(List<CoordinateData> coords,
			List<Double> radialLength, List<Double> magneticStrength,
			List<Double> ns, List<Double> bs, List<Double> mp, List<Double> lv,
			List<Float> il, List<SpaceRegion> sr, List<FootpointRegion> rtr,
			List<FootpointRegion> nbtr, List<FootpointRegion> sbtr,
	...
	private static void print(List<XMLGregorianCalendar> time,
			...
			List<Double> magneticStrength, List<Double> ns, List<Double> bs,
			List<Double> mp, List<Double> lv, List<Float> il,
			List<SpaceRegion> sr, List<FootpointRegion> rtr,
			List<FootpointRegion> nbtr, List<FootpointRegion> sbtr,
			List<Double> bGseX, List<Double> bGseY, List<Double> bGseZ,
			int numValues) {
		...
		for (int i = 0; i < time.size() && i < numValues; i++) {
			...
			if (magneticStrength != null && i < magneticStrength.size()) {
			...
			if (sbtr != null && i < sbtr.size()) {
				...
				System.out.printf("  %10s", getTracedRegion(sbtr.get(i)));
			}
	...
	private static DataFileRequest dataRequest(XMLGregorianCalendar startDate, XMLGregorianCalendar endDate, String satelliteId) {
		...
		MappedRegionFilterOptions radialTraceRegionsFilter = new MappedRegionFilterOptions();
		...
		MappedRegionFilterOptions magneticTraceRegionsFilter = radialTraceRegionsFilter;
		// magnetic traced regions filter

		RegionFilterOptions regionFilters = new RegionFilterOptions();
		// region filter
		...
		BFieldModelParameters externalBFieldModel = new BFieldModelParameters();
	...
	private static OutputOptions getTestOutputOptions() {
		...
		for (CoordinateComponent component : CoordinateComponent.values()) {
			...
			option.setCoordinateSystem(CoordinateSystem.GEO);
	...
	private static MapProjectionGraphOptions getTestMapOptions() {
		...
		MapProjectionGraphOptions options = new MapProjectionGraphOptions();
	...
	private static TimeSeriesGraphOptions getTestTimeSeriesOptions() {
		...
		TimeSeriesGraphOptions options = new TimeSeriesGraphOptions();

		List<CoordinateOptions> coordOptions = new ArrayList<CoordinateOptions>();
		...
		List<BFieldTraceOptions> traceOptions = new ArrayList<BFieldTraceOptions>();
